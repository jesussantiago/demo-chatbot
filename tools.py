...
query_sql_query_def = {
    ...
    "function": {
        "name": "query_sql_queries",
        "description": "Queries the PostgreSQL dvdrental database to retrieve information about films, actors, customers, rentals, and more.",
        "parameters": {
            "type": "object",
            "properties": {
                "sql_queries": {
                    ...
                    "items": {
                        "type": "string",
                        "description": """An array of well-written PostgreSQL SELECT statements to extract data from the dvdrental database.

Main tables you can use:
- **film**: Information about available movies (columns: film_id, title, description, release_year, rental_rate, rating).
- **customer**: Customer records (columns: customer_id, first_name, last_name, email, address_id).
- **rental**: Rental transactions (columns: rental_id, rental_date, customer_id, inventory_id, return_date).
- **payment**: Payment transactions (columns: payment_id, customer_id, staff_id, rental_id, amount, payment_date).
- **actor**: List of actors (columns: actor_id, first_name, last_name).
- **category**: Categories for films.
- **inventory**: Mapping of films to stores.
- **store**, **address**, **city**, **country**: Related to locations and branches.

Examples of valid queries:
- Count the number of customers in the system.
- List the top 5 most rented movies.
- Get the total revenue per customer.
- Show the number of films per rating category.

Important notes:
- Use **LIMIT** to restrict results.
- You can use aggregation functions like COUNT, AVG, SUM.
- Use **JOINs** between tables for richer queries (e.g., rental JOIN customer JOIN payment).
